...
module.exports = {
    ...
    insertDataTestId: (fixer, node) => {
        const { skipAutofixFor } = defaults;
        const nodeName = node && node.name && node.name.name;

        // Can't do anything if the node name doesn't exist
        if (!nodeName) {
            return;
        }
        ...
        if (node.attributes.length > 0 &&
            node.attributes.some(n => n.name && n.name.name === 'data-test-id')) {
            return;
        }

        // Do not add test ids to these plain HTML tags
        if (nodeName && skipAutofixFor.includes(nodeName)) {
            return;
        }


        const dataTestIdToken = ` data-test-id="${ dashCase(nodeName) }"`;
